...
#include <fstream>
    ...
    RusGame::RusGame(sf::Font& font, std::function<void(Screen*)> switchScreenCallback, std::string difficulty, std::wstring playerName)
        : font(font), switchScreenCallback(switchScreenCallback), difficultyLevel(difficulty), rng(std::random_device{}()), playerName(playerName), inputFieldHandler(inputText, cursorVisible, cursorTimer)
    {
        if (difficulty == "easy") {
    ...
    bool RusGame::isValidWord(const std::wstring& word) {
        ...
        if (word.find(requiredPart) == std::wstring::npos) return false;
        return russianWords.count(word) > 0;
    }

    void RusGame::endGame() {
        std::wcout << L"Игра окончена! Ваш счёт: " << score << std::endl;

        std::wofstream file("rating.txt", std::ios::app);
        if (file.is_open()) {
            file << playerName << L" - " << score << L" очков\n";
            file.close();
        }
    ...
    void RusGame::loadRussianWords(const std::string& filename) {
        std::wifstream file(filename);
        file.imbue(std::locale("")); // системная локаль

        if (!file.is_open()) {
            std::cout << "Failed to open russian.txt" << std::endl;
            return;
        }

        std::wstring word;
        size_t count = 0;
        const size_t logInterval = 100000;

        while (std::getline(file, word)) {
            if (!word.empty()) {
                russianWords.insert(word);
                count++;
        ...
        std::cout << "Finished loading. Total words: " << count << std::endl;
    }
